...
def predict():
    ...
    with open('/app/app/models/model_columns.txt', 'r') as filehandle:
        for line in filehandle:
            currentPlace = line[:-1]
            col_list.append(currentPlace)
    ...
    for _ in col_list:
      if request_json[_]:
        newdf[_] = [request_json[_]]
        __ = newdf[_]
        logger.info(f'{_}={__}')
		logger.info(f'EndData')
    
		try:
			preds = model.predict(newdf)
